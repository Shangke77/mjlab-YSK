...
def _symmetric_gui_limit(bounds: tuple[float, float]) -> float:
  """Return a nonzero symmetric GUI limit containing the configured range."""
  return max(0.1, abs(bounds[0]), abs(bounds[1]))
...
class UniformVelocityCommand(CommandTerm):
  ...
  def create_gui(
    ...
    axes = [
      ("lin_vel_x", _symmetric_gui_limit(ranges.lin_vel_x)),
      ("lin_vel_y", _symmetric_gui_limit(ranges.lin_vel_y)),
      ("ang_vel_z", _symmetric_gui_limit(ranges.ang_vel_z)),
    ]
    sliders: list = []
